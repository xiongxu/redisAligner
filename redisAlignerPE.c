//gcc -g -O3 -Wall klib/kthread.c redis_query_kthread_block_uniqPE.c -o redis_query_kthread_block_uniqPE -I./klib -I./hiredis -L./hiredis -lhiredis -lpthread -lz -lm
// awk -F"\t" '{hash[$NF]++}END{for (e in hash){print e"\t"hash[e]}}'|sort -k1,1n
#include "IO_stream.h"
#include <getopt.h>
#include <inttypes.h>
#include <math.h>
...
void *worker_pipeline(void *shared, int step, void *in) // kt_pipeline() callback
{
  ...
  if (step == 0) { // step 0: read lines into the buffer
    ...
    s->n_lines=0;
    s->lines = calloc(p->max_lines * 2, sizeof(sds));
    s->c = p->c;
    s->groupCount = calloc(p->n_threads, sizeof(int));
    s->AccumulatedCount = calloc(p->n_threads, sizeof(int));
    while (readNextNode(p->fp1,p->buf1,p->buf_size,s->lines+s->n_lines) && readNextNode(p->fp2,p->buf2,p->buf_size,s->lines+s->n_lines+1)){
      p->TRC++;
      if (hasN(*(s->lines+s->n_lines)) || strlen(p->buf1) < 37 || hasN(*(s->lines+s->n_lines+1)) || strlen(p->buf2) < 37){
        sdsfree(*(s->lines+s->n_lines));
        sdsfree(*(s->lines+s->n_lines+1));
        continue;
      }
      s->n_lines+=2;
      if (s->n_lines >= p->max_lines * 2) break;
    }
    if (s->n_lines) {
    ...
    step_t *s = (step_t*)in;
    if (s->n_lines) p->NoN_TRC += s->n_lines/2;
    for (i=0;i<s->n_lines;i+=2) {
      if (s->sraBlock.a[i].chr && s->sraBlock.a[i+1].chr){
        s->sraBlock.a[i].iSize=abs(s->sraBlock.a[i].pos-s->sraBlock.a[i+1].pos)+36;
        if (s->sraBlock.a[i].iSize >=50 && s->sraBlock.a[i].iSize<=1000) p->iSizeFreq[s->sraBlock.a[i].iSize]++;
        kv_push(SRA, p->sraArray, s->sraBlock.a[i]);
      }else if(s->sraBlock.a[i].chr){
        kv_push(SRA, p->sraArray, s->sraBlock.a[i]);
      }else if(s->sraBlock.a[i+1].chr){
        s->sraBlock.a[i+1].read=1;
        kv_push(SRA, p->sraArray, s->sraBlock.a[i+1]);
      }
    }
    free(s->codeLines);
    free(s->groupCount);
...
int main(int argc, char *argv[]) {
  ...
  const char *optString = "1:2:o:r:c:b:s:vdh?";
  ...
  pipeline_t pl;
  pl.fp1=open_input_stream(globalArgs.infile1);
  pl.fp2=open_input_stream(globalArgs.infile2);
  int pl_threads =6,steps=6;
  uint32_t i=0,j=0,k=0;
  pl.buf_size = 0x200;
  pl.max_lines = globalArgs.readCount;
  pl.buf1 = calloc(pl.buf_size, 1);
  pl.buf2 = calloc(pl.buf_size, 1);
  pl.n_threads = 12;
  pl.iSizeFreq = calloc(1001,sizeof(uint32_t));
  pl.TRC=pl.NoN_TRC=0;
  kv_init(pl.sraArray);
  ...
  if (verbose){
    for (i=0;i<pl.sraArray.n;++i){
      fprintf(stdout,"%" PRIu8 "\t%" PRIu32 "\t%" PRIu8 "\t%c\t%" PRIu8 "\n",pl.sraArray.a[i].chr,pl.sraArray.a[i].pos,pl.sraArray.a[i].GC,pl.sraArray.a[i].strand?'+':'-',pl.sraArray.a[i].iSize);
    }
  }
  FILE *iSizeFreqOut = fcreat_outfile(globalArgs.outfile,".iSizeFreq.tsv");
  for (i=50;i<1001;++i){
    fprintf(iSizeFreqOut,"%d",i);fputc(i<1000?'\t':'\n',iSizeFreqOut);
  }
  for (i=50;i<1001;++i){
    fprintf(iSizeFreqOut,"%" PRIu32 ,pl.iSizeFreq[i]);fputc(i<1000?'\t':'\n',iSizeFreqOut);
  }
  fclose(iSizeFreqOut);
  if (dedup){
    ...
    fprintf(stderr,"redundency ratio: %" PRIu64 " / %" PRIu64 " = %%%lf\n", redundency,pl.NoN_TRC,1.0*redundency/pl.NoN_TRC*100);
  }else{
    for (i=0;i<pl.sraArray.n;++i){
      uint8_t chrIndex=pl.sraArray.a[i].chr-1;
      pl.RC[chrIndex][0]++;
  ...
  for (i=0;i<24;++i){
    fprintf(pl.fout,"%d\t%lf\t%" PRIu64 "\t%lf\t%lf\t%" PRIu64 "\t%lf\t%lf\t%" PRIu64 "\t%f\n",i+1,
    ...
    fprintf(blkOut, "%d\t%zu",i+1,continousBin[i].n);
